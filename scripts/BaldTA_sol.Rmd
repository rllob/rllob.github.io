...
```{r}
# load data
dat <- read.csv(file="data/bald_TA.csv")
...
```{r}
m1 <- lm(approve_bald_TA ~ polisci, data = dat)

summary(m1)


m2 <- lm(approve_bald_TA ~ econ, data = dat)

summary(m2)


m3 <- lm(approve_bald_TA ~ soc, data = dat)
...
```{r}
m1 <- lm(approve_bald_TA ~ polisci + econ, data = dat)

summary(m1)


m2 <- lm(approve_bald_TA ~ polisci + soc, data = dat)

summary(m2)


m3 <- lm(approve_bald_TA ~ econ + soc, data = dat)
...
Again, perhaps stargazer will make the comparisons easier. What do you see? Why are the results changing? How can you interpret these results in a consitent manner?
...
Finally, regress the following model:


$$ approval_i = \alpha + \beta_1 age_i + \beta_2 soc_i + \beta_3 female_i + e_i$$



```{r}
m1 <- lm(approve_bald_TA ~ age, data = dat)
m2 <- lm(approve_bald_TA ~ age + soc, data = dat)
m3 <- lm(approve_bald_TA ~ age + soc + female, data = dat)
```
